...
def plot_line():
    ...
    df_one_well['month'] = df_one_well['date'].dt.month
    monthly_avg = df_one_well.groupby('month')['v_oil'].mean()

    angles = np.linspace(0, 2 * np.pi, 12, endpoint=False)
    values = [monthly_avg.get(m, 0) for m in range(1, 13)]

    bars = ax.bar(angles, values, width=2 * np.pi / 12, alpha=0.8,
                  color=plt.cm.viridis(np.array(values) / max(values)))

    ax.set_xticks(angles)
    ax.set_xticklabels(['Янв', 'Фев', 'Мар', 'Апр', 'Май', 'Июн',
                        'Июл', 'Авг', 'Сен', 'Окт', 'Ноя', 'Дек'])
    ax.set_title(f'Сезонность добычи (скважина {CURRENT_ID})')
...
def plot_bar():
    ...
    ax.bar(x_labels, values, color=['green', 'orange', 'red'] if BAR_GROUP == "risk" else ['pink', 'blue', 'lime', 'yellow'])
...
def plot_scatter():
    ...
    if SCATTER_COLOR == "risk":
        colors = {'low': 'green', 'medium': 'yellow', 'high': 'pink'}
        point_colors = df_one_well['risk'].map(colors)
        legend_title = "Риск"
        legend_elements = [
            Patch(facecolor='green', label='low'),
            Patch(facecolor='yellow', label='medium'),
            Patch(facecolor='pink', label='high')
        ]
    else:  # season
        season_colors = {'зима': 'blue', 'весна': 'pink', 'лето': 'green', 'осень': 'orange'}
        point_colors = df_one_well['season'].map(season_colors)
        legend_title = "Сезон"
        legend_elements = [
            Patch(facecolor='blue', label='зима'),
            Patch(facecolor='pink', label='весна'),
            Patch(facecolor='green', label='лето'),
...
def plot_heat_map():
    ...
    sns.heatmap(pivot_data, annot=False, cmap='spring', ax=ax)
...
df_raw = pd.read_csv('data_unnorm.csv')
